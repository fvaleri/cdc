...
oc exec -it my-mysql-ss-0 -- sh -c 'mysql -u root < /config/initdb.d/initdb.sql'

# do some changes
oc exec -it my-mysql-ss-0 -- sh -c 'MYSQL_PWD="cdcadmin" mysql -u cdcadmin cdcdb -e \
...
# check status
oc exec -it my-mysql-ss-0 -- sh -c 'MYSQL_PWD="cdcadmin" mysql -u cdcadmin cdcdb -e "SELECT * FROM customers"'
```
...
# deploy resources
oc create secret generic connectors-prop --from-file=./openshift/streams/connect/connectors.properties
...
oc create -f ./openshift/streams/connect/mysql-source.yaml
oc create -f ./openshift/streams/connect/amq-sink.yaml
